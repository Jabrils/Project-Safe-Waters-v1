...
Shader "Lux Water/Underwater/Transparent Premultiply" {
	Properties {
		...
		[NoScaleOffset] _FinalAlpha 	("    Final Alpha", 2D) = "white" {}
	...
	SubShader {
		...
		CGPROGRAM
		#pragma surface surf StandardSpecularLuxUnderwater fullforwardshadows 	nofog   vertex:vert  alpha:premul  nometa  keepalpha
		#pragma target 3.0
	...
	//	Enable proper dialectric fresnel 
		#pragma shader_feature GEOM_TYPE_LEAF

		#define USEALPHAPREMULTIPLY
	...
	//	Particle Lighting
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecularLuxUnderwater o) {
			...
			#if defined(GEOM_TYPE_BRANCH)
				o.Alpha.y = tex2D (_FinalAlpha, IN.uv_MainTex).a * IN.color.a;
			#endif
